...
fighter_data <- fighter_data %>%
  mutate(period = floor((year(date) - 1994) / 5))  
...
period_labels <- paste0(1994 + 5 * (0:(length(fighter_list) - 1)), "-", 1998 + 5 * (0:(length(fighter_list) - 1)))
names(fighter_list) <- period_labels
```
...
(fighter_period_data <- fighter_list %>%
  map_dfr(~ tibble(
    Unique_Fighters = n_distinct(.x$fighter),
    Total_Rows = nrow(.x)
  ), .id = "Period"))
...
imputed_data_list <-  vector(mode = "list",length = 6)
...
imputed_data_list[[4]] <- fighter_list[[periods_for_analysis[4]]] %>% dplyr::select(is.numeric, -period)
imputed_data_list[[5]] <- fighter_list[[periods_for_analysis[5]]] %>% dplyr::select(is.numeric, -period)
imputed_data_list[[6]] <- fighter_list[[periods_for_analysis[6]]] %>% dplyr::select(is.numeric, -period)
```

```{r}
var_explained_list <- vector(mode = "list",length = 6)

data_pca_res_list <- vector(mode = "list",length = 6)
...
```{r}
kaisers_rules <- vector(mode = "list",length = 6)
...
(kaisers_rules[4] <- length(which(data_pca_res_list[[4]]$sdev^2 > 1)))
(kaisers_rules[5] <- length(which(data_pca_res_list[[5]]$sdev^2 > 1)))
(kaisers_rules[6] <- length(which(data_pca_res_list[[6]]$sdev^2 > 1)))
```

```{r}
L_thresholded_list <- vector(mode = "list",length = 6)
...
```{r}
hiartigans_list <- vector(mode="list",length=6)
for(i in 1:6){
  hiartigans_list[[i]] <- FitKMeans(imputed_data_list[[i]], max.clusters=30, seed=507)
}
```
...
PlotHartigan(hiartigans_list[[3]])
PlotHartigan(hiartigans_list[[4]])
PlotHartigan(hiartigans_list[[5]])
PlotHartigan(hiartigans_list[[6]])
```

3 seems reliable for each

```{r}
imputed.kmeans.list <- vector(mode="list", length=6)

for(i in 1:6){
  set.seed(507)
  imputed.kmeans.list[[i]] <- kmeans(imputed_data_list[[i]], centers = 3)
}
```

```{r}
cluster_colmeans_list <- vector(mode="list", length=6)
