---
title: "ufc_player_factor_cluster_pca_analysis"
format: html
editor: visual
---

## Set up

This is similar to the ufc_factor_cluster_pca analysis but on player stats

```{r}
rm(list = ls())
gc()
library(tidyverse)
library(lubridate)
library(MASS)
library(mult)
library(glue)
```

### With NA omitted data

```{r}
na_omit <- read_csv("Data/na_omitted_data.csv") %>%
  # anything with 3 or less unique values should be a factor
  mutate(across(where(is.numeric), ~ if(length(unique(.)) <= 3) factor(.) else .))
```

```{r}
red_fighter_data <- na_omit %>%
  dplyr::select(fighter = R_fighter, date, starts_with("R_")) %>%
  rename_with(~str_remove(., "R_"), starts_with("R_"))

blue_fighter_data <- na_omit %>%
  dplyr::select(fighter = B_fighter, date, starts_with("B_")) %>%
  rename_with(~str_remove(., "B_"), starts_with("B_"))

fighter_data <- bind_rows(red_fighter_data, blue_fighter_data)

fighter_data <- fighter_data %>% arrange(date)

head(fighter_data)
```

```{r}

# creating datasets consisfting of fighter data for a given period of 5 years.

fighter_data <- fighter_data %>%
  mutate(period = floor((year(date) - 1995) / 5))  

# I need only the fighters most recent stats in a given 5 year period 

fighter_data_recent <- fighter_data %>%
  group_by(period, fighter) %>%
  arrange(desc(date), .by_group = TRUE) %>%
  slice(1) %>%  
  ungroup()

# split into different datasets for each period

fighter_list <- fighter_data_recent %>%
  group_split(period)

period_labels <- paste0(1995 + 5 * (0:(length(fighter_list) - 1)), "-", 1999 + 5 * (0:(length(fighter_list) - 1)))
names(fighter_list) <- period_labels
```

```{r}
# make sure no duplicate fighters

(fd <- fighter_list %>%
  map_dfr(~ tibble(
    Unique_Fighters = n_distinct(.x$fighter),
    Total_Rows = nrow(.x)
  ), .id = "Period"))
```

```{r}
fd %>% tail(3)
```

No duplicate fighters but years 1995-2004 have barely anybody, will continue with years after.

```{r}
periods_for_analysis <- c("2010-2014","2015-2019","2020-2024")

na_omit_data_list <-  vector(mode = "list",length = 3)

na_omit_data_list[[1]] <- fighter_list[[periods_for_analysis[1]]] %>% dplyr::select(is.numeric, -period)
na_omit_data_list[[2]] <- fighter_list[[periods_for_analysis[2]]] %>% dplyr::select(is.numeric, -period)
na_omit_data_list[[3]] <- fighter_list[[periods_for_analysis[3]]] %>% dplyr::select(is.numeric, -period)
```

```{r}
var_explained_list <- vector(mode = "list",length = 3)

data_pca_res_list <- vector(mode = "list",length = 3)

for(i in 1:length(na_omit_data_list)){
  
  data_pca_res_list[[i]] <- prcomp(na_omit_data_list[[i]],
                         center=T, scale.=T)
  
  var_explained_list[[i]] <- data_pca_res_list[[i]]$sdev^2 / sum(data_pca_res_list[[i]]$sdev^2)
  
  print(which(cumsum(var_explained_list[[i]]) >= 0.9)[1])
  
}
```

```{r}

for(i in 1:3){
  
  plot(var_explained_list[[i]], xlab="# of Principal Component", ylab = "Proportion of Variance Explained", main = glue("Skree Plot for {periods_for_analysis[[i]]}"), ylim=c(0,1), type='b', col="blue", pch=19, lty=1)

  grid(lty=1, nx = NA, ny = NULL)

  lines(cumsum(var_explained_list[[i]]), type='b', col="red", pch=19, lty=2)
  legend("right", legend=c("Proportion of Variance Explained", "Cumulative Proportion of Variance Explained"), col=c("blue", "red"), pch=19, lty=1:2, xpd=TRUE)
  
}

```

```{r}
(kaisers_rule1 <- length(which(data_pca_res_list[[1]]$sdev^2 > 1)))
(kaisers_rule2 <- length(which(data_pca_res_list[[2]]$sdev^2 > 1)))
(kaisers_rule3 <- length(which(data_pca_res_list[[3]]$sdev^2 > 1)))
```

Same rule for each.

```{r}
L_thresholded_list <- vector(mode = "list",length = 3)

for(i in 1:3){
  eigenvectors_1_to_kaiser<- data_pca_res_list[[i]]$rotation[,1:kaisers_rule1]
  eigenvalues_1_to_kaiser <- c(data_pca_res_list[[i]]$sdev^2)[1:kaisers_rule1]
  
  L <- eigenvectors_1_to_kaiser %*% diag(sqrt(eigenvalues_1_to_kaiser))
  
  print(mean(sort(rowSums(L^2))))
  
  Psi <- diag(diag(cor(na_omit_data_list[[i]]) - L %*% t(L)))
  
  thresholded_L <- apply(L, 2,
                         function(col) ifelse(col >= 0.6 | col <= -0.6, col, "."))
  
  thresholded_L <- as.data.frame(thresholded_L)
  
  thresholded_L <- thresholded_L[apply(thresholded_L,
                                       1, function(row) any(row != ".")), ]
  
  L_thresholded_list[[i]] <- thresholded_L[, apply(thresholded_L,
                                         2, function(col) any(col != "."))]
}
```

```{r}
library(useful)

hiartigans_list <- vector(mode="list",length=3)
for(i in 1:3){
  hiartigans_list[[i]] <- FitKMeans(data_pca_res_list[[i]]$x[,1:23], max.clusters=30, seed=507)
}
```

```{r}
par(mfrow = c(2, 2))
PlotHartigan(hiartigans_list[[1]])
PlotHartigan(hiartigans_list[[2]])
PlotHartigan(hiartigans_list[[3]])
par(mfrow = c(1, 1))
```

4 seems a reliable result for each.

```{r}
naomit.kmeans.list <- vector(mode="list", length=3)

for(i in 1:3){
  set.seed(507)
  naomit.kmeans.list[[i]] <- kmeans(data_pca_res_list[[i]]$x[,1:23], centers = 4)
}
```

```{r}
cluster_colmeans_list <- vector(mode="list", length=3)

for(i in 1:3){
  
  ndata <- na_omit_data_list[[i]]
  
  ndata$Cluster <- naomit.kmeans.list[[i]]$cluster

  cluster_colmeans_list[[i]] <- ndata %>%
    group_by(Cluster) %>%
    summarise(
       cluster_count = n(),
      across(everything(), mean, .names = "mean_{.col}")
      )
}
```

```{r}
highlights <- c("Cluster","cluster_count","mean_avg_KD","mean_avg_KD","mean_avg_CTRL_timeseconds","mean_avg_CTRL_timeseconds")

highlights2 <- c("mean_Weight_lbs","mean_Weight_lbs","mean_avg_SIG_STR_att","mean_avg_SIG_STR_att","mean_avg_TOTAL_STR_landed","mean_avg_TOTAL_STR_landed")
cluster_colmeans_list[[3]][,highlights]
```

```{r}
cluster_colmeans_list[[3]]
```

```{r}
library(gridExtra)
library(ggplot2)
library(grDevices)

plots <- list()

for (i in 1:3) {
  pca_data <- as.data.frame(data_pca_res_list[[i]]$x[, 1:2])  
  pca_data$Cluster <- as.factor(naomit.kmeans.list[[i]]$cluster)

  n_clusters <- length(unique(pca_data$Cluster))

  distinct_colors <- colorRampPalette(c(
    "#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", 
    "#FF7F00", "#FFFF33", "#A65628", "#F781BF",
    "#999999", "#66C2A5", "#FC8D62", "#8DA0CB", "#E78AC3"
  ))(n_clusters)

  distinct_shapes <- c(16, 17, 18, 19, 15, 8, 1, 2, 3, 4, 5, 6, 7)[1:n_clusters]

  p <- ggplot(pca_data, aes(x = PC1, y = PC2, color = Cluster, shape = Cluster)) +
    geom_point(size = 2.5) +  
    scale_color_manual(values = distinct_colors) +  
    scale_shape_manual(values = distinct_shapes) + 
    labs(title = paste("K-Means Clustering for: ", periods_for_analysis[[i]], sep=""),
         color = "Cluster", shape = "Cluster") +
    theme_minimal()

  plots[[i]] <- p
}

grid.arrange(grobs = plots, ncol = 2)
```

```{r}
imputed <- read_csv("Data/data_numeric_imputed.csv") %>%
  mutate(across(where(is.numeric), ~ if(length(unique(.)) <= 3) factor(.) else .))

imputed_numeric <- imputed %>% 
  dplyr::select(where(is.numeric))
```

```{r}
red_fighter_data <- imputed %>%
  dplyr::select(fighter = R_fighter, date, starts_with("R_")) %>%
  rename_with(~str_remove(., "R_"), starts_with("R_"))

blue_fighter_data <- imputed %>%
  dplyr::select(fighter = B_fighter, date, starts_with("B_")) %>%
  rename_with(~str_remove(., "B_"), starts_with("B_"))

fighter_data <- bind_rows(red_fighter_data, blue_fighter_data)

fighter_data <- fighter_data %>% arrange(date)

head(fighter_data)
```

```{r}
#creating datasets consisfting of fighter data for a given period of 5 years.

fighter_data <- fighter_data %>%
  mutate(period = floor((year(date) - 2010) / 5))  

#I need only the fighters most recent stats in a given 5 year period 

fighter_data_recent <- fighter_data %>%
  group_by(period, fighter) %>%
  arrange(desc(date), .by_group = TRUE) %>%
  slice(1) %>%  
  ungroup()

#split into different datasets for each period

fighter_list <- fighter_data_recent %>%
  group_split(period)

period_labels <- paste0(2010 + 5 * (0:(length(fighter_list) - 1)), "-", 2014 + 5 * (0:(length(fighter_list) - 1)))
names(fighter_list) <- period_labels
```

```{r}
# make sure no duplicate fighters

(fd2 <- fighter_period_data <- fighter_list %>%
  map_dfr(~ tibble(
    Unique_Fighters = n_distinct(.x$fighter),
    Total_Rows = nrow(.x)
  ), .id = "Period"))
```

```{r}
periods_for_analysis <- fighter_period_data$Period

imputed_data_list <-  vector(mode = "list",length = 3)

imputed_data_list[[1]] <- fighter_list[[periods_for_analysis[1]]] %>% dplyr::select(is.numeric, -period)
imputed_data_list[[2]] <- fighter_list[[periods_for_analysis[2]]] %>% dplyr::select(is.numeric, -period)
imputed_data_list[[3]] <- fighter_list[[periods_for_analysis[3]]] %>% dplyr::select(is.numeric, -period)
```

```{r}
var_explained_list <- vector(mode = "list",length = 3)

data_pca_res_list <- vector(mode = "list",length = 3)

for(i in 1:length(imputed_data_list)){
  
  data_pca_res_list[[i]] <- prcomp(imputed_data_list[[i]],
                         center=T, scale.=T)
  
  var_explained_list[[i]] <- data_pca_res_list[[i]]$sdev^2 / sum(data_pca_res_list[[i]]$sdev^2)
  
  print(which(cumsum(var_explained_list[[i]]) >= 0.9)[1])
  
}
```

```{r}
for(i in 1:3){
  
  plot(var_explained_list[[i]], xlab="# of Principal Component", ylab = "Proportion of Variance Explained", main = glue("Skree Plot for {periods_for_analysis[[i]]}"), ylim=c(0,1), type='b', col="blue", pch=19, lty=1)

  grid(lty=1, nx = NA, ny = NULL)

  lines(cumsum(var_explained_list[[i]]), type='b', col="red", pch=19, lty=2)
  legend("right", legend=c("Proportion of Variance Explained", "Cumulative Proportion of Variance Explained"), col=c("blue", "red"), pch=19, lty=1:2, xpd=TRUE)
  
}
```

```{r}
kaisers_rules <- vector(mode = "list",length = 3)

(kaisers_rules[1] <- length(which(data_pca_res_list[[1]]$sdev^2 > 1)))
(kaisers_rules[2] <- length(which(data_pca_res_list[[2]]$sdev^2 > 1)))
(kaisers_rules[3] <- length(which(data_pca_res_list[[3]]$sdev^2 > 1)))
```

```{r}
L_thresholded_list <- vector(mode = "list",length = 3)

for(i in 1:3){
  eigenvectors_1_to_kaiser<- data_pca_res_list[[i]]$rotation[,1:kaisers_rules[[i]]]
  eigenvalues_1_to_kaiser <- c(data_pca_res_list[[i]]$sdev^2)[1:kaisers_rules[[i]]]
  
  L <- eigenvectors_1_to_kaiser %*% diag(sqrt(eigenvalues_1_to_kaiser))
  Psi <- diag(diag(cor(imputed_data_list[[i]]) - L %*% t(L)))
  
  thresholded_L <- apply(L, 2,
                         function(col) ifelse(col >= 0.6 | col <= -0.6, col, "."))
  
  thresholded_L <- as.data.frame(thresholded_L)
  
  thresholded_L <- thresholded_L[apply(thresholded_L,
                                       1, function(row) any(row != ".")), ]
  
  L_thresholded_list[[i]] <- thresholded_L[, apply(thresholded_L,
                                         2, function(col) any(col != "."))]
}
```

```{r}
hiartigans_list <- vector(mode="list",length=3)
for(i in 1:3){
  hiartigans_list[[i]] <- FitKMeans(imputed_data_list[[i]], max.clusters=30)
}
```

```{r}
PlotHartigan(hiartigans_list[[1]])
PlotHartigan(hiartigans_list[[2]])
PlotHartigan(hiartigans_list[[3]])
```

3 seems reliable for each

```{r}
imputed.kmeans.list <- vector(mode="list", length=3)

for(i in 1:3){
  set.seed(507)
  imputed.kmeans.list[[i]] <- kmeans(imputed_data_list[[i]], centers = 4)
}
```

```{r}
cluster_colmeans_list <- vector(mode="list", length=3)

for(i in 1:3){
  
  ndata <- imputed_data_list[[i]]
  
  ndata$Cluster <- imputed.kmeans.list[[i]]$cluster

  cluster_colmeans_list[[i]] <- ndata %>%
    group_by(Cluster) %>%
    summarise(
       cluster_count = n(),
      across(everything(), mean, .names = "mean_{.col}")
      )
}
```

```{r}
cluster_colmeans_list[[1]]
```

```{r}
highlights <- c("Cluster","cluster_count","mean_avg_KD","mean_avg_KD","mean_avg_CTRL_timeseconds","mean_avg_CTRL_timeseconds")

highlights2 <- c("mean_Weight_lbs","mean_Weight_lbs","mean_avg_SIG_STR_att","mean_avg_SIG_STR_att","mean_avg_TOTAL_STR_landed","mean_avg_TOTAL_STR_landed")
cluster_colmeans_list[[1]][,highlights]
```

```{r}
cluster_colmeans_list[[3]][,highlights]
```
